...
import { CoffeeList, CoffeesProps } from "./components/CoffeeList";
import { Intro } from "./components/Intro";

export interface SelectedCoffesProps {
    id: number;
    name: string;
    price: string;
    quantity: number;
...
export function Home() {
    ...
    const [selectCoffees, setSelectCoffees] = useState<SelectedCoffesProps[]>([]);

    function handleAddCoffee(coffee: CoffeesProps) {
        if (!selectCoffees.find(item => item.id == coffee.id)) {
            return setSelectCoffees([
                ...selectCoffees,
                {
                    id: coffee.id,
                    name: coffee.name,
        ...
        const newSelectCoffees = selectCoffees.map(item => {
            return {
                ...item,
                quantity: item.id == coffee.id ? item.quantity + 1 : item.quantity,
            }
        });

        setSelectCoffees(newSelectCoffees);
    }
